...
CM_M1.connect_one_to_one(weights=parameters['strength_m1_copy'])
...
for i, subset_key in enumerate(parameters['subsets_str']):
    ...
    StrD1_SNr[subset_key] = ann.Projection(pre=StrD1[:, :, interval[0]:interval[1]], post=SNr[:, i],
                                           target='inh', synapse=PreCovariance_inhibitory, name=f'D1_SNr_{subset_key}')
    StrD1_SNr[subset_key].connect_all_to_all(0.0)
...
# Feedback connection
M1_StrD1 = {}
w_M1_str = w_one_to_ones(preDim=parameters['dim_motor'][0], postDim=tuple(list(parameters['dim_s1']) + [parameters['dim_motor'][0]]), weight=0.5)
for i, subset_key in enumerate(parameters['subsets_str']):
    interval = parameters['subsets_str'][subset_key]
    M1_StrD1[subset_key] = ann.Projection(pre=M1[:, i], post=StrD1[:, :, interval[0]:interval[1]], target='exc', name=f"M1_StrD1_{subset_key}")

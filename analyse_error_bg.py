...
for sim in sims:
    for root, dirs, _ in os.walk(f'results/test_run_model_{sim}'):
        for d in dirs:
            if sim == sims[0]:
                error[d] = []
...
n_training_trails = (1000, 2000, 4000, 8000, 16000, 32000)
...
axs[0].set_ylim(0, 70), axs[1].set_ylim(0, 70)
...
plt.savefig('error.pdf', dpi=300, bbox_inches='tight', pad_inches=0)
plt.show()